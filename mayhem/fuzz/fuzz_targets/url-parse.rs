  ...
  match target {
    ...
    Target::UrlReparse(href, (setter_idx, value)) => {
      if let Ok(mut parsed) = url::Url::parse(&href) {
        match setter_idx % 9 {
          0 => {
            url::quirks::set_hash(&mut parsed, &value);
          }
          1 => {
            let _ = url::quirks::set_host(&mut parsed, &value);
          }
          2 => {
            let _ = url::quirks::set_hostname(&mut parsed, &value);
          }
          3 => {
            let _ = url::quirks::set_password(&mut parsed, &value);
          }
          4 => {
            url::quirks::set_pathname(&mut parsed, &value);
          }
          5 => {
            let _ = url::quirks::set_port(&mut parsed, &value);
          }
          6 => {
            let _ = url::quirks::set_protocol(&mut parsed, &value);
          }
          7 => {
            url::quirks::set_search(&mut parsed, &value);
          }
          _ => {
            let _ = url::quirks::set_username(&mut parsed, &value);
          }
        }
      }
    }
